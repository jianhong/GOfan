...
sunburstGO <- function(df,
    ...
    vs_df <- filterGraph(g, cutoff=GO_annotation_level_cutoff,
    ...
    if(plotBy=='plotly'){
        ...
        pl <- plot_ly(vs_df, ids = as.formula("~offspring"),
        ...
        if('fontsize' %in% names(args)){
            if(is.character(args$fontsize)){
                if(args$fontsize %in% colnames(df)){
                    if(args$fontsize %in% c(colnames(plotdata),
                                       "polar_angle", "rotate90")){
                        stop(args$fontsize, ' is a column of plotdata.',
